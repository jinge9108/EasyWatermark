...
class LaunchView : CustomViewGroup {
    ...
    val ivTakePhoto: MaterialButton by lazy {
        MaterialButton(context).apply {
            minHeight = 56.dp
            minWidth = 120.dp
            cornerRadius = 56.dp / 3
            textAlignment = TEXT_ALIGNMENT_CENTER
            gravity = Gravity.CENTER
            text = context.getString(R.string.tips_take_photo)
            shapeAppearanceModel = ShapeAppearanceModel.Builder().also {
                it.setAllCornerSizes(0f)
            }.build()
        }
    ...
    private val launchViews by lazy {
        listOf(logoView, ivSelectedPhotoTips, ivTakePhoto, ivGoAboutPage)
    }
...
//        setBackgroundColor(ContextCompat.getColor(context, R.color.md_theme_dark_background))
    ...
    private fun layoutLaunch() {
        ...
        ivSelectedPhotoTips.let {
            it.layoutCenterHorizontal(appendY = (measuredHeight * 0.55f).toInt())
        }
        ivTakePhoto.let {
            it.layoutCenterHorizontal(appendY = (measuredHeight * 0.65f).toInt())
        }
        ivGoAboutPage.let {
